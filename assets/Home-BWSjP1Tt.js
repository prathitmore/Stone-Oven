import{W as L,A as B,S as Y,F as D,P as H,a as N,b as R,V as $,M as O,B as F,c as k,d as _,e as X,C as q,o as W}from"./three.module-DRDOGXzl.js";import{i as j}from"./tilt-fQ6J8_vA.js";function U(){return`
    ...
    `}function V(){const m=document.getElementById("hero-canvas");if(!m)return;const h=window.innerWidth<=768,v=new L({canvas:m,alpha:!0,antialias:!h,powerPreference:"high-performance"});v.setSize(window.innerWidth,window.innerHeight),v.setPixelRatio(Math.min(window.devicePixelRatio,h?1:1.5)),v.toneMapping=B,v.toneMappingExposure=1;const C=new Y;C.fog=new D(1115136,.015);const u=new H(h?70:50,window.innerWidth/window.innerHeight,.1,1e3);u.position.set(0,10,40);const x=h?48:80,e=new N(120,120,x,x),t=new R({uniforms:{uTime:{value:0},uMouse:{value:new $(0,0)}},wireframe:!1,vertexShader:`
            uniform float uTime;
            uniform vec2 uMouse;
            varying vec2 vUv;
        ...
        `,transparent:!0}),i=new O(e,t);i.rotation.x=-Math.PI/2+.2,i.position.y=-5,i.position.z=-20,C.add(i);const n=h?150:600,r=new F,g=new Float32Array(n*3),a=new Float32Array(n),f=new Float32Array(n);for(let o=0;o<n;o++)g[o*3]=(Math.random()-.5)*80,g[o*3+1]=(Math.random()-.5)*20,g[o*3+2]=(Math.random()-.5)*60-10,a[o]=Math.random()*.8+.2,f[o]=Math.random()*100;r.setAttribute("position",new k(g,3)),r.setAttribute("aScale",new k(a,1)),r.setAttribute("aLife",new k(f,1));const s=new R({transparent:!0,depthWrite:!1,blending:_,uniforms:{uTime:{value:0}},vertexShader:`
                ...
                <div style="text-align: center; margin-bottom: 6vw;">
                    ...
                    ${W.slice(0,4).map((m,h)=>`
                        <div class="destination-card reveal" data-tilt style="position: relative; height: 75vh; overflow: hidden; border-radius: 4px; cursor: pointer;">
                            <img src="${m.images.hero}" style="width: 100%; height: 100%; object-fit: cover; transition: transform 0.8s cubic-bezier(0.16, 1, 0.3, 1);" alt="${m.name}">
                            <div class="card-overlay"></div>
                            <div class="card-content-inner" style="position: absolute; bottom: 4rem; left: 4rem; right: 4rem;">
                                <div style="color: #ff3c00; font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.6em; margin-bottom: 12px; font-weight: 800;">Destination 0${h+1}</div>
                                <h3 style="font-family: 'Playfair Display', serif; font-size: 3.5rem; color: #fff; margin-bottom: 0.8rem; line-height: 1;">${m.name.split(" ").pop()}</h3>
                                <p style="color: rgba(255,255,255,0.6); font-size: 0.9rem; letter-spacing: 0.05em; max-width: 300px; line-height: 1.6;">${m.address}</p>
                                <div style="margin-top: 2.5rem;">
                                    <a href="#/outlet/${m.id}" class="btn-premium-outline" style="padding: 1rem 2rem; font-size: 0.75rem; background: rgba(255,255,255,0.05); backdrop-filter: blur(5px);">Explore Outlet</a>
                                </div>
                            </div>
                        </div>
                    `).join("")}
    ...
    `}function J(){const m=window.innerWidth<=768;V(),j();const h=document.getElementById("heritage-img-wrap"),v=document.getElementById("heritage-card-3d");h&&v&&(h.addEventListener("mousemove",e=>{const t=h.getBoundingClientRect(),i=(e.clientX-t.left)/t.width-.5,n=(e.clientY-t.top)/t.height-.5;v.style.animation="none",v.style.transform=`rotateX(${-n*18}deg) rotateY(${i*18}deg) translateY(-8px)`}),h.addEventListener("touchstart",e=>{const t=e.touches[0],i=h.getBoundingClientRect(),n=(t.clientX-i.left)/i.width-.5,r=(t.clientY-i.top)/i.height-.5;v.style.animation="none",v.style.transform=`rotateX(${-r*22}deg) rotateY(${n*22}deg) translateY(-12px)`},{passive:!0}),h.addEventListener("mouseleave",()=>{v.style.animation="float3d 6s ease-in-out infinite",v.style.transform=""}),h.addEventListener("touchend",()=>{v.style.animation="float3d 6s ease-in-out infinite",v.style.transform=""},{passive:!0})),document.querySelectorAll(".destination-card").forEach(e=>{e.addEventListener("mousemove",t=>{const i=e.getBoundingClientRect(),n=(t.clientX-i.left)/i.width-.5,r=(t.clientY-i.top)/i.height-.5,g=e.querySelector("img"),a=e.querySelector(".card-content-inner");g&&(g.style.transform=`scale(1.15) translate(${-n*30}px, ${-r*30}px) rotate(${n*2}deg)`),a&&(a.style.transform=`translate3d(${n*25}px, ${r*25}px, 50px)`),e.style.transform=`rotateX(${-r*10}deg) rotateY(${n*10}deg)`}),e.addEventListener("touchstart",t=>{const i=t.touches[0],n=e.getBoundingClientRect(),r=(i.clientX-n.left)/n.width-.5,g=(i.clientY-n.top)/n.height-.5,a=e.querySelector("img"),f=e.querySelector(".card-content-inner");e.classList.add("touch-active"),a&&(a.style.transform=`scale(1.2) translate(${-r*40}px, ${-g*40}px) rotate(${r*4}deg)`),f&&(f.style.transform=`translate3d(${r*35}px, ${g*35}px, 60px)`),e.style.transform=`rotateX(${-g*15}deg) rotateY(${r*15}deg) scale(1.02)`},{passive:!0}),e.addEventListener("mouseleave",()=>{const t=e.querySelector("img"),i=e.querySelector(".card-content-inner");t&&(t.style.transform=""),i&&(i.style.transform=""),e.style.transform=""}),e.addEventListener("touchend",()=>{const t=e.querySelector("img"),i=e.querySelector(".card-content-inner");e.classList.remove("touch-active"),t&&(t.style.transform=""),i&&(i.style.transform=""),e.style.transform=""},{passive:!0})});const u=document.getElementById("cta-embers");if(u){let r=function(){const s=u.parentElement.getBoundingClientRect();t=u.width=s.width,i=u.height=s.height},f=function(){document.getElementById("cta-embers")&&(e.clearRect(0,0,t,i),e.shadowBlur=0,n.forEach(s=>{s.update(),s.draw()}),requestAnimationFrame(f))};const e=u.getContext("2d");let t,i;const n=[];window.addEventListener("resize",r),r();class g{constructor(){this.reset()}reset(){this.x=Math.random()*t,this.y=i+50,this.vx=(Math.random()-.5)*2,this.vy=-Math.random()*3-2,this.size=Math.random()*(m?2.5:3.5)+1,this.alpha=Math.random()*.7+.3,this.life=Math.random()*(m?150:250)+100,this.color=Math.random()>.5?"#ff3c00":"#ff9d00"}update(){this.x+=this.vx+Math.sin(this.y*.01)*.5,this.y+=this.vy,this.life--,this.alpha-=m?.004:.002,(this.life<=0||this.alpha<=0||this.y<-20)&&this.reset()}draw(){e.globalAlpha=this.alpha,e.fillStyle=this.color,m||(e.shadowBlur=12,e.shadowColor=this.color),e.beginPath(),e.arc(this.x,this.y,this.size,0,Math.PI*2),e.fill()}}const a=m?200:750;for(let s=0;s<a;s++)n.push(new g);f()}const x=document.getElementById("masterpiece-showcase");if(x){let w=function(l){const d=G[l];if(!d)return;a=l,M&&clearTimeout(M);const c=document.querySelector(".masterpiece-info");c&&(c.style.opacity="0",c.style.transform="translateY(15px)",c.style.transition="all 0.3s cubic-bezier(0.16, 1, 0.3, 1)"),n&&(n.style.opacity="0",n.style.transform="scale(0.92) rotate(-8deg)",n.style.transition="all 0.3s cubic-bezier(0.16, 1, 0.3, 1)"),M=setTimeout(()=>{if(e&&(e.textContent=d.name),t&&(t.textContent=d.description),i&&(i.textContent=`0${l+1}`),n){const p=new Image;p.src=d.image,p.onload=()=>{n.src=d.image,c&&(c.style.opacity="1",c.style.transform="translateY(0)"),n&&(n.style.opacity="1",n.style.transform="scale(1) rotate(0deg)")}}},200),r&&(r.style.transform=`translateX(${(l-1)*-15}vw) translateZ(0)`),g&&(g.style.transform=`translate(${(l-1)*-80}px, ${(l-1)*50}px) rotate(${(l-1)*15}deg) translateZ(0)`)};const e=document.getElementById("masterpiece-title"),t=document.getElementById("masterpiece-desc"),i=document.getElementById("masterpiece-num"),n=document.getElementById("masterpiece-img"),r=document.getElementById("masterpiece-bg-text"),g=document.getElementById("masterpiece-ingredients");let a=0,f=0,s=!1,M=null,b=0,z=0;const T=()=>{if(!document.getElementById("masterpiece-showcase"))return;if(s&&document.body.style.overflow!=="hidden"&&(document.body.style.overflow="hidden"),s){Math.abs(window.scrollY-z)>10&&window.scrollTo(0,z);return}if(Date.now()-b<1e3)return;const d=x.getBoundingClientRect(),c=window.pageYOffset+d.top,p=m?window.innerHeight*.5:window.innerHeight*.4;if(d.top<p&&d.top>-p){if(s=!0,z=c,m){const y=document.querySelector(".navbar");y&&y.classList.add("nav-hidden")}document.body.style.overflow="hidden",document.documentElement.style.overflow="hidden",window.scrollTo(0,c)}},S=l=>{if(!document.getElementById("masterpiece-showcase")){s&&(s=!1,document.body.style.overflow="");return}if(!s)return;const d=Date.now(),c=l.deltaY>0,p=l.deltaY<0;if(a===0&&p||a===2&&c)if(d-f>1100){s=!1;const y=document.querySelector(".navbar");y&&y.classList.remove("nav-hidden"),b=d,document.body.style.overflow="",document.documentElement.style.overflow="";return}else{l.preventDefault();return}l.preventDefault(),!(d-f<1300)&&(c&&a<2?(f=d,w(a+1)):p&&a>0&&(f=d,w(a-1)))};window.removeEventListener("scroll",window._homeScrollHandler),window.removeEventListener("wheel",window._homeWheelHandler),window._homeScrollHandler=T,window._homeWheelHandler=S,window.addEventListener("scroll",T),window.addEventListener("wheel",S,{passive:!1});let A=0;x.addEventListener("touchstart",l=>{A=l.touches[0].clientY},{passive:!0}),x.addEventListener("touchmove",l=>{document.getElementById("masterpiece-showcase")&&s&&l.preventDefault()},{passive:!1}),x.addEventListener("touchend",l=>{if(!s)return;const d=l.changedTouches[0].clientY,c=A-d,p=Date.now();if(Math.abs(c)<40)return;const y=c>0,I=c<0,o=m?600:1300;if((a===0&&I||a===2&&y)&&p-f>o){s=!1;const E=document.querySelector(".navbar");E&&E.classList.remove("nav-hidden"),b=p,document.body.style.overflow="",document.documentElement.style.overflow="";return}p-f<o||(y&&a<2?(f=p,w(a+1)):I&&a>0&&(f=p,w(a-1)))}),m||x.addEventListener("mousemove",l=>{const{clientX:d,clientY:c}=l,p=(d/window.innerWidth-.5)*40,y=(c/window.innerHeight-.5)*40;pizzaWrap&&(pizzaWrap.style.transform=`translate3d(${p}px, ${y}px, 0) rotateX(${-y/2}deg) rotateY(${p/2}deg)`),r&&(r.style.transform=`translate(${-p/2}px, ${-y/2}px)`)}),w(0)}}export{J as init,Q as renderHome};
