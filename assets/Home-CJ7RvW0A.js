import{W as L,A as B,S as Y,F as D,P as H,a as N,b as R,V as $,M as O,B as F,c as k,d as _,e as X,C as q,o as W}from"./three.module-DRDOGXzl.js";import{i as j}from"./tilt-fQ6J8_vA.js";function U(){return`
    ...
    `}function V(){const f=document.getElementById("hero-canvas");if(!f)return;const g=window.innerWidth<=768,v=new L({canvas:f,alpha:!0,antialias:!g,powerPreference:"high-performance"});v.setSize(window.innerWidth,window.innerHeight),v.setPixelRatio(Math.min(window.devicePixelRatio,g?1:1.5)),v.toneMapping=B,v.toneMappingExposure=1;const M=new Y;M.fog=new D(1115136,.015);const h=new H(g?70:50,window.innerWidth/window.innerHeight,.1,1e3);h.position.set(0,10,40);const w=g?48:80,t=new N(120,120,w,w),i=new R({uniforms:{uTime:{value:0},uMouse:{value:new $(0,0)}},wireframe:!1,vertexShader:`
        ...
        `,transparent:!0}),e=new O(t,i);e.rotation.x=-Math.PI/2+.2,e.position.y=-5,e.position.z=-20,M.add(e);const r=g?150:600,n=new F,m=new Float32Array(r*3),y=new Float32Array(r),l=new Float32Array(r);for(let o=0;o<r;o++)m[o*3]=(Math.random()-.5)*80,m[o*3+1]=(Math.random()-.5)*20,m[o*3+2]=(Math.random()-.5)*60-10,y[o]=Math.random()*.8+.2,l[o]=Math.random()*100;n.setAttribute("position",new k(m,3)),n.setAttribute("aScale",new k(y,1)),n.setAttribute("aLife",new k(l,1));const c=new R({transparent:!0,depthWrite:!1,blending:_,uniforms:{uTime:{value:0}},vertexShader:`
        ...
        `}),x=new X(n,c);M.add(x);let b=0,z=0,E=0,T=0,S=0,A=0;const a=window.innerWidth/2,d=window.innerHeight/2,s=(o,C)=>{b=(o-a)*.002,z=(C-d)*.002,S=o/window.innerWidth,A=1-C/window.innerHeight;const P=document.getElementById("hero-light");P&&(P.style.background=`radial-gradient(circle 800px at ${o}px ${C}px, rgba(255, 60, 0, 0.06), transparent)`)};document.addEventListener("mousemove",o=>s(o.clientX,o.clientY)),document.addEventListener("touchstart",o=>{o.touches.length>0&&s(o.touches[0].clientX,o.touches[0].clientY)},{passive:!0}),document.addEventListener("touchmove",o=>{o.touches.length>0&&s(o.touches[0].clientX,o.touches[0].clientY)},{passive:!0});const p=new q;function u(){requestAnimationFrame(u);const o=p.getElapsedTime();E=b*2,T=z*2,h.position.x+=(E-h.position.x)*.02,h.position.y+=(10+T-h.position.y)*.02,h.lookAt(0,5,-20),i.uniforms.uTime.value=o,i.uniforms.uMouse.value.x+=(S-i.uniforms.uMouse.value.x)*.05,i.uniforms.uMouse.value.y+=(A-i.uniforms.uMouse.value.y)*.05,c.uniforms.uTime.value=o,v.render(M,h),window.hero3DReady||(window.hero3DReady=!0)}u();const I=()=>{h.aspect=window.innerWidth/window.innerHeight,h.updateProjectionMatrix(),v.setSize(window.innerWidth,window.innerHeight)};window.removeEventListener("resize",window._heroResizeHandler),window._heroResizeHandler=I,window.addEventListener("resize",I)}const G=[{id:"dish-1",name:"Butter Chicken Pizza",description:"Tender tandoori chicken in rich makhani sauce on a stone-fired base.",image:"masterpiece_pizza_1.png"},{id:"dish-2",name:"Mexican Paneer Pizza",description:"Spicy paneer chunks, jalapeños, and bell peppers.",image:"masterpiece_pizza_2.png"},{id:"dish-3",name:"BBQ Chicken Pizza",description:"Smoky BBQ sauce reduction, soft paneer chunks.",image:"masterpiece_pizza_3.png"}];function Q(){return`
        <div class="home-page">
            ${U()}
